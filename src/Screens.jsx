...
export function StartScreen({ start, ret, setRet }) {
  useGSAP(() => {
    if (ret) {
      gsap.to('.split', { y: "-100%", duration: 0.5, stagger: {
        each: 0.1,
        ease: 'power1.inOut'
      }})
    }    
  ...
  setTimeout(() => {setRet(false)}, 600)

  return (
    <>
    {ret ? 
    <>
    <div className="bg-red-200 w-1/4 h-screen fixed top-0 left-0 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-0 left-1/4 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-0 left-2/4 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-0 left-3/4 z-50 split"></div>
    </> : null
  }
...
export function SelectScreen({
  ...
  const transition = contextSafe(() => {
    ...
    tl.to('.choose', { y: 120, opacity: 0, duration: 0.7, ease: 'back.inOut(5)'})
  ...
  if (players && difficulty) {
    setTimeout(select, 600)
    transition();
  }

  return (
    <div className="w-full min-h-screen h-fit bg-green-50 px-8 py-24">
      <h1 className="font-semibold text-2xl text-blue-700 text-center mb-8 flex items-center justify-center gap-2 header">
        Select Your Gameplay
        <img src={gamePad} className="w-8 h-8 relative" />
      </h1>
      <div className="bg-white rounded-lg p-8 flex flex-col justify-between gap-4 shadow-md  max-w-[24rem] m-auto choose">
...
export function PlayScreen({
  ...
  const transition = contextSafe(() => {
    ...
    gsap.to('.pageTrans', { y: -1000, duration: 0.8, ease: 'power3.inOut(10)', stagger: {
      each: 0.1,
      ease: 'power1.inOut'
    }})
  })

  const transition2 = contextSafe(() => {
    gsap.to('.split', { y: "-100%", duration: 0.5, stagger: {
      each: 0.1,
      ease: 'power1.inOut'
    }})
  ...
  return (
    ...
    <>
    <div className="bg-green-50 w-screen h-screen fixed top-0 z-50 pageTrans"></div>
    <div className="bg-blue-100 w-screen h-screen fixed top-0 z-40 pageTrans"></div>
    <div className="bg-blue-200 w-screen h-screen fixed top-0 z-30 pageTrans"></div>

    <div className="bg-red-200 w-1/4 h-screen fixed top-full left-0 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-full left-1/4 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-full left-2/4 z-50 split"></div>
    <div className="bg-red-200 w-1/4 h-screen fixed top-full left-3/4 z-50 split"></div>
    <div
      className={
    ...
    >
      ...
      <div
        onClick={() => {setTimeout(end, 600); transition2(); setRet(true)}}
...
export function StatsScreen({ winner, end, gameMode, setRet }) {
    ...
    return (
      ...
      <div className="w-full h-screen flex flex-col items-center justify-center">
        ...
        <button
          ...
          onClick={() => {setTimeout(end, 600); transition(); setRet(true)}}
        >
          Return to Start
        </button>
      </div>
